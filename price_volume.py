...
def create_candlestick_volume_chart(
    ...
    try:
        ...
        print(f"Failed to render candlestick chart: {e}")
        fig.add_trace(
            go.Scatter(
                x=df.index,
                y=df["Close"],
...
def draw_profile_of_data(ohlc_df: pd.DataFrame, ticker: str) -> None:
    ...
    all_index_dates = set(ohlc_df.index.date)
    ...
    if len_all_index_dates == 1:
        ...
        chart_file_name = f"volume_profile/profile_{ticker}_{index_date}.png"
    else:
        chart_file_name = f"volume_profile/profile_{ticker}_{len_all_index_dates}d.png"
...
if __name__ == "__main__":
    ...
    print(data.head())
    print(data.tail())
    sorted_dates = sorted(list(set(data.index.date)))
    if len(sorted_dates) != 5:
        raise ValueError(
            f"We ask Yahoo Finance for 5 days of data, so len should be 5, {sorted_dates=}"
        )
    
    data_slice: pd.DataFrame = data[data.index.date == sorted_dates[-1]]
    draw_profile_of_data(ohlc_df=data_slice, ticker=TICKER)
    
    data_slice: pd.DataFrame = data[data.index.date == sorted_dates[-2]]
    draw_profile_of_data(ohlc_df=data_slice, ticker=TICKER)
    
    data_slice: pd.DataFrame = data[data.index.date == sorted_dates[-3]]
    draw_profile_of_data(ohlc_df=data_slice, ticker=TICKER)
